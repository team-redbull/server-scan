...
async def test_fleet_snapshot_counts_stale_unreachable_and_last_seen(
    ...
    for server in (
        _server("fresh-1", provider="OPENMANAGE", seen_ago=timedelta(hours=1)),
        _server("fresh-2", provider="OPENMANAGE", seen_ago=timedelta(hours=2)),
        _server("old-1", provider="OPENMANAGE", seen_ago=timedelta(days=3), reachable=False),
        _server("never-1", provider="OPENMANAGE", seen_ago=None),
...
async def test_fleet_snapshot_groups_clusters_health_and_maintenance(
    ...
    for server in (
        ...
        _server("c", provider="FAKE", seen_ago=timedelta(0), health=HealthSeverity.CRITICAL),
    ):
        await repo.upsert(server)
