# CLAUDE.md archive — the full text moved out on 2026-09-13

CLAUDE.md is loaded into every session's context, and Anthropic's own
guidance is to keep it under ~200 lines. On 2026-09-13 it was 1,660 lines,
so three sections were condensed in place and their **verbatim** text
moved here, per convention 8 (never delete a hard-won fact — move it with
its provenance). Nothing here was rewritten. The condensed versions, and
the path-scoped rules under `.claude/rules/`, are what a session reads now;
this file is the reference when one of those pointers is not enough.

Each section below is exactly as it stood in CLAUDE.md at that moment,
including its own dated corrections. Facts here can go stale like any
note — the ADR each paragraph names is the authoritative record.

---

# Section: Current status (as of 2026-09-13)
...
# Section: Key technical facts (as of 2026-09-13)
...
- **`get_one(ServerIdentity)` is a sixth abstract method on
  `ServerInventoryProvider`, for a live single-server recheck — read
  `docs/adr/0032-available-server-lookup-api.md` before adding a new
  vendor or touching an existing one.** `ServerIdentity` carries
  `serial`/`external_id`/`host`/`name`; each provider reads only the
  field it correlates on, and fetches that one server directly — a
  scoped `$filter` (Intersight), a direct-by-URI/DN read (OneView, and
  UCS Manager's own `query_dn(hierarchy=True)`), a single-host recollect
  (Redfish standalone, and OpenManage via a one-off Redfish target), or a
  small directory query plus a per-domain delegation (UCS Central).
  **Never `_list_servers()` re-run and filtered** — that would make a
  live recheck cost what a whole collection run costs. `tools.run_collector.
  build_provider_for_manager_type` is the one place that turns a
  `ManagerType` into a constructed provider for this path, shared with
  the CLI's own resolution, so there is exactly one answer to "how does a
  manager type become a provider instance" regardless of caller.
  `FakeProvider` gained a test-only `get_one_overrides` constructor
  param (default empty, never set in production wiring) because its
  fleet is otherwise fully deterministic and could never on its own
  produce the "a live recheck downgrades a candidate" scenario the test
  suite has to prove.
- **The API pod reads vendor manager credentials, for the first time
  since ADR-0012 made that exclusively the collectors' concern.**
  `GET /api/v1/servers/available`'s live recheck needs `get_one()` to
  actually call a manager, so `backend-deployment.yaml` now mounts the
  same `<release>-collector-credentials` Secret every CronJob does — not
  behind a values toggle, because an unconfigured value already means
  "skip the recheck, trust Mongo" (`ManagerNotConfiguredError`), the
  identical degrade a collector already performs when misconfigured. The
  one thing **not** mounted onto the API pod is `REDFISH_STANDALONE`'s
  per-host TOML inventory/credentials files — that would put every
  standalone BMC's password within reach of the pod the Route exposes,
  for a freshness guarantee this endpoint already treats as best-effort
  everywhere else. That collector's candidates always trust Mongo.
...
# Section: the dev stack — measured Podman/compose behaviour (2026-09-05)

Moved out of CLAUDE.md's "Verifying your work"; the short version there
is what to *do*, this is what was *measured* and why.
